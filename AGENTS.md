...
9. Verify remote Render health/CORS/auth/calibration/browser flow.
10. Only then resume BridgeAdapter.

## Final verdict
**NOT READY.**
Reason: clean retraining not yet runtime-verified; current checkpoint quality not trusted; current full test suite not run after latest changes; browser/Render behavior not live-tested; raw-video production API boundary remains incomplete.
