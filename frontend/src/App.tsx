...
function History() { const [data, setData] = useState<any>(); const [query, setQuery] = useState(""); const [modelFilter, setModelFilter] = useState(""); const [exporting, setExporting] = useState(false); const [exportError, setExportError] = useState(""); useEffect(() => { const param = modelFilter ? `&used_adapter=${modelFilter}` : ""; api.history(`range=all&sort=newest${param}`).then(setData).catch(() => setData({ items: [] })); }, [modelFilter]); const rows = data?.items?.filter((x: any) => !query || String(x.predicted_text).toLowerCase().includes(query.toLowerCase())) || [];
  async function downloadCsv() {
    ...
    try {
      ...
      link.href = url; link.download = "visionbridge-history.csv";
      document.body.appendChild(link); link.click(); link.remove();
      URL.revokeObjectURL(url);
    } catch (err) { setExportError(err instanceof Error ? err.message : "Export failed"); }
    finally { setExporting(false); }
  }
  return <Page title="History" subtitle="Searchable translation events persisted for your account."><section className="panel"><div className="panel-head"><div><div className="eyebrow">EVENTS</div><h2>History</h2></div><div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}><select className="compact-input" value={modelFilter} onChange={e => setModelFilter(e.target.value)}><option value="">All models</option><option value="true">Personalized only</option><option value="false">Base model only</option></select><input className="compact-input" placeholder="Search translations" value={query} onChange={e => setQuery(e.target.value)} /><button type="button" className="ghost-btn" onClick={downloadCsv} disabled={exporting}>{exporting ? "Exporting…" : "Export CSV"}</button></div></div>{exportError && <div className="alert error">{exportError}</div>}{!data ? <Loading /> : rows.length ? <div className="table-wrap"><table><thead><tr><th>Time</th><th>Prediction</th><th>Model</th><th>Confidence</th><th>Latency</th></tr></thead><tbody>{rows.map((row: any) => <tr key={row.id}><td>{new Date(row.created_at).toLocaleString()}</td><td>{row.predicted_text || "(no sign detected)"}</td><td>{row.used_adapter ? "Personalized" : "Base model"}</td><td>{Math.round((row.confidence || 0) * 100)}%</td><td>{Math.round(row.latency_ms || 0)} ms</td></tr>)}</tbody></table></div> : <Empty text="No matching events." />}</section></Page>; }
