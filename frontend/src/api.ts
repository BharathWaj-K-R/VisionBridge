...
export const api = {
  ...
  exportHistoryCsv: async (): Promise<Blob> => {
    if (LOCAL_MODE) {
      ...
      const note = "# Demo mode — connect a real backend (VITE_LOCAL_MODE=false) to export real history.\n";
      return new Blob([note, header], { type: "text/csv" });
    }
    // /history/export.csv requires Bearer auth — a plain <a href> download
    // never sends the token stored in localStorage, so this goes through
    // the same authenticated request() path as everything else and turns
    // the response into a Blob for a client-side download instead.
