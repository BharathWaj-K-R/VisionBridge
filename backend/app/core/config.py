...
class Settings:
    ...
    # Both endpoints run real model inference/training per request — the
    # single most expensive work this API does. In-process sliding-window
    # limiter (see app/core/rate_limit.py); appropriate for this app's
    # actual deployment shape (one backend instance on Render, no shared
    # cache/Redis to coordinate a distributed limiter across).
    TRANSLATE_RATE_LIMIT_PER_MINUTE: int = int(
        os.getenv("TRANSLATE_RATE_LIMIT_PER_MINUTE", "60")
    )
    CALIBRATION_RATE_LIMIT_PER_MINUTE: int = int(
