...
def list_my_adapters(db:Session=Depends(get_db),current_user:User=Depends(get_current_user)):
    rows=(db.query(SignerAdapter).filter(SignerAdapter.owner_id==current_user.id).order_by(SignerAdapter.created_at.desc()).all())
    return [row for row in rows if Path(row.weights_path).name.startswith("letter_adapter_")]

@router.delete("/me/adapters/{adapter_id}")
def delete_my_adapter(adapter_id:int,db:Session=Depends(get_db),current_user:User=Depends(get_current_user)):
