...
_calibration_limiter = SlidingWindowRateLimiter(limit=settings.CALIBRATION_RATE_LIMIT_PER_MINUTE)
_rate_limit = make_rate_limit_dependency(_calibration_limiter)
...
@router.post("", response_model=CalibrationResult, dependencies=[Depends(_rate_limit)])
def calibrate(
    payload: CalibrationRequest,
